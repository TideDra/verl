...
def load_corpus(corpus_path: str):
    ...
    # {'text': xxx, 'meta': xxx}
    return corpus  # testing


def get_top_doc(text):
    indent = 0
    pos = -1
...
class DenseRetriever(BaseRetriever):
    def __init__(self, config):
        ...
        self.chunk_merge_method = config.chunk_merge_method
    ...
    def _search(self, query: str, num: int = None, return_score: bool = False):
        ...
        idxs = idxs[0]
        scores = scores[0]
        results = load_docs(self.corpus, idxs, chunk_merge_method = self.chunk_merge_method)
        if return_score:
    ...
    def _batch_search(self, query_list: List[str], num: int = None, return_score: bool = False):
        ...
        for start_idx in tqdm(range(0, len(query_list), self.batch_size), desc="Retrieval process: "):
            ...
            batch_results = load_docs(self.corpus, flat_idxs, chunk_merge_method = self.chunk_merge_method)
            # chunk them back
...
class Config:
    ...
    def __init__(
        ...
        self.chunk_merge_method = chunk_merge_method
